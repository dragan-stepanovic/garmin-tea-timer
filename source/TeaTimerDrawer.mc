...
class TeaTimerDrawer {
    function draw(dc, currentTeaTimer, currentTeaTimerIndex, teaTypes) {
        dc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_BLACK);
        dc.clear();

        drawProgressArc(dc, currentTeaTimer);
    ...
    function formatTimeString(currentTeaTimer) {
        var minutesAndSeconds = currentTeaTimer.timeRemaining.minutesAndSecondsLeft();
        return minutesAndSeconds[0] + ":" + minutesAndSeconds[1].format("%02d");
    }

    function drawProgressArc(dc, currentTeaTimer) {
    ...
    function drawTeaName(dc, currentTeaTimer) {
        dc.setColor(currentTeaTimer.color, Graphics.COLOR_BLACK);
        dc.drawText(
            dc.getWidth() / 2,
            dc.getHeight() / 2 - 106,
    ...
    function drawTimer(dc, currentTeaTimer) {
        var timeString = formatTimeString(currentTeaTimer);
    ...
    function drawTeaIcon(dc, currentTeaTimer, currentTeaTimerIndex) {
        var centerX = dc.getWidth() / 2;
        var centerY = dc.getHeight() / 2 - 165;

        if (currentTeaTimerIndex == 0) {
            // Jasmine
            drawJasmineFlower(dc, currentTeaTimer, centerX, centerY);
        } else if (currentTeaTimerIndex == 1) {
            // Mint
            drawMintLeaves(dc, currentTeaTimer, centerX, centerY);
        } else if (currentTeaTimerIndex == 2) {
            // Earl Grey
            drawSteamLines(dc, currentTeaTimer, centerX, centerY);
        } else if (currentTeaTimerIndex == 3) {
            // Green Tea
            drawTeaLeaf(dc, currentTeaTimer, centerX, centerY);
        }
    }

    function drawJasmineFlower(dc, currentTeaTimer, centerX, centerY) {
        var petalRadius = 6;
    ...
    function drawMintLeaves(dc, currentTeaTimer, centerX, centerY) {
        dc.setColor(currentTeaTimer.color, Graphics.COLOR_BLACK);
        dc.setPenWidth(3);
    ...
    function drawSteamLines(dc, currentTeaTimer, centerX, centerY) {
        dc.setColor(currentTeaTimer.color, Graphics.COLOR_BLACK);
        dc.setPenWidth(2);
    ...
    function drawTeaLeaf(dc, currentTeaTimer, centerX, centerY) {
        dc.setColor(currentTeaTimer.color, Graphics.COLOR_BLACK);
        dc.setPenWidth(3);
    ...
    function drawPageDots(dc, currentTeaTimerIndex, teaTypes) {
        var baseDotRadius = 5;
        var dotSpacing = 16;
